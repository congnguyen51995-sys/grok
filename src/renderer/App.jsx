...
export default function App({ onLicenseExpired }) {
  ...
  const [updatePhase,    setUpdatePhase]    = useState('idle'); // idle | downloading | ready | error | snoozed
  ...
  useEffect(() => {
    // Có bản mới → main đã tự bắt đầu tải, chỉ cần hiển thị trạng thái
    ...
    window.electronAPI?.onUpdateDownloadError?.(({ error }) => {
      ...
      setUpdateError(error || 'Tải thất bại — đang thử lại...');
    });
  ...
  const handleSnooze = () => {
    if (countdownRef.current) clearInterval(countdownRef.current);
    setUpdatePhase('snoozed');
  };

  // Giữ lại để IPC handler update:download vẫn hoạt động khi cần
  const handleDownloadUpdate = async () => {
    setUpdatePhase('downloading');
    setUpdateError('');
    setUpdateProgress(0);
    const res = await window.electronAPI?.downloadUpdate?.();
    if (res?.success) setUpdatePhase('ready');
    else { setUpdatePhase('error'); setUpdateError(res?.error || 'Tải thất bại'); }
  };
  ...
  return (
    <div className={`h-screen flex flex-col ${bg0} transition-colors duration-200`}
      ...
      {/* ── OVERLAY BẮT BUỘC CẬP NHẬT — chặn toàn màn hình ─────────────── */}
      {updateInfo && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center"
          style={{ background: 'radial-gradient(ellipse at center, #0f0c29 0%, #06040f 100%)' }}>
          ...
          <div className="relative z-10 flex flex-col items-center gap-6 px-8 py-10 max-w-sm w-full text-center">
            {/* Logo / Icon */}
            ...
            {/* Title */}
            <div>
              <p className="text-xs font-semibold text-violet-400 uppercase tracking-widest mb-1">Cập nhật bắt buộc</p>
              <h1 className="text-2xl font-black text-white">
                Fluxy <span className="text-violet-400">v{updateInfo.newVersion}</span>
              </h1>
              {updateInfo.releaseNotes && (
                <p className="mt-2 text-sm text-slate-400 leading-relaxed">{updateInfo.releaseNotes}</p>
              )}
            </div>
            ...
            <div className="w-full">
              {updatePhase === 'downloading' && (
                <>
                  ...
                  <p className="text-sm text-slate-400">
                    {updateProgress > 0
                      ? <><span className="text-white font-bold">{updateProgress}%</span> — Đang tải bản mới...</>
              ...
              {updatePhase === 'ready' && (
                <>
                  <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden mb-2">
                    <div className="h-2 rounded-full w-full bg-emerald-500" />
                  </div>
                  <p className="text-sm text-emerald-400 font-semibold">
                    ✅ Tải xong — đang cài đặt...
                  </p>
                </>
              )}
              ...
              {updatePhase === 'error' && (
                <div className="bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3">
                  <p className="text-sm text-red-400">⚠ Lỗi tải — đang thử lại tự động...</p>
                  <p className="text-xs text-slate-500 mt-1">{updateError}</p>
                </div>
              )}
            </div>

            {/* Spinning loader */}
            {(updatePhase === 'downloading' || updatePhase === 'ready') && (
      ...
      <header className="h-14 bg-black flex items-center px-4 gap-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 border-r border-slate-800 pr-4 mr-1 shrink-0">
          ...
          <div className="flex flex-col leading-tight">
            ...
            {appVersion && (
              <span className="text-[8px] font-bold text-slate-600 leading-none mt-0.5 tracking-wider">v{appVersion}</span>
            )}
          </div>
        </div>
